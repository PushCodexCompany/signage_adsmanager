...
const New_Booking_Steps_Modal = (props) => {
    ...
    const { children, options = [], currentStep = 1, handleNextStep = () => { }, handleBackStep = () => { }, setStep = () => { } } = props;
            ...
            <div className="p-4  pl-16 flex space-x-3 border-b-1 border-gray-300">
                {
                    options.map((step, index) => {
                        ...
                        return (
                            <div key={`step_${index}`} className="flex-1 flex  items-center justify-center" onClick={() => { handleStepClicked(step.stepIndex) }} style={{ cursor: (step.stepIndex < currentStep) ? "pointer" : "default" }}>
                                <div className={`flex items-center justify-center w-10 h-10 bg-${isActiveStep ? ACTIVE_STEP_BG_COLOR : INACTIVE_STEP_BG_COLOR} rounded-full mr-4`}>
                                    <p className={`text-${isActiveStep ? ACTIVE_STEP_LABEL_COLOR : INACTIVE_STEP_LABEL_COLOR}  font-poppins text-lg`}>{step.stepIndex}</p>
                                </div>
                                <div className="flex items-center justify-center">
                                    <div className="font-poppins text-2xl ">{step.label}</div>
                                </div>
                                <div className="flex flex-1 items-center justify-center">
                                    {(index < (options.length - 1)) ? <div className="font-poppins text-2xl ">{`>`}</div> : null}
                                </div>
                            </div>
                        )
                    })
