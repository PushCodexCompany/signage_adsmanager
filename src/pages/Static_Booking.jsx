...
const Static_Booking = () => {
  ...
  const getBookingData = async () => {
    if (searchTerm === null) {
      const data = await User.getBooking(token, 1, "", "", 3);
      ...
      if (filter_screen.length > 0) {
        ...
        data = await User.getBooking(token, 1, searchTerm, obj, 3);
      } else {
        data = await User.getBooking(token, 1, searchTerm, "", 3);
      }
      setBookingData(data.booking);
      if (data.pagination.length > 0) {
        setAllPages(data.pagination[0].totalpage);
